...
TEST(TaskTest, RopeGapTest) {
    ...
    EXPECT_NEAR(calculateGapAfterRope(earthRadius, ropeIncrease), 0.1592, 0.0001);
}
...
TEST(TaskTest, PoolCostTest) {
    ...
    double totalCost = calculateCostOfPoolAndPath(poolRadius, pathWidth, concreteCost, fenceCost);
